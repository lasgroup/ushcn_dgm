...
class Simulator(ABC):
    ...
    def simulate_trajectory(self, initial_condition: jnp.array, times: jnp.array, sigma: Optional[float] = None,
                            key: Optional[jnp.array] = None) -> Tuple[jnp.array, jnp.array, jnp.array]:
    ...
    def simulate_trajectories(self, initial_conditions, times, sigmas, rng) -> Tuple[
        ...
        """
        each List has in its first dimension the state dimension ID
        as second argument the trajectory
        and as third the respective time index.

        this is necessery, so that we can have different numbers of observations for different state dimensions.
        call return_argument[dimension_id][trajectory_id][time_point_id]
        """
        trajectories = []
        ground_truth_states = []
        ground_truth_derivatives = []
        num_trajectories = len(initial_conditions)
        num_dimensions = len(initial_conditions[0])
        rng, *key = jax.random.split(rng, num_dimensions*num_trajectories + 1)
        for dimension_id in range(num_dimensions):
            current_trajectory = []
            current_ground_truth_state = []
            current_ground_truth_derivative = []
            for trajectory_id in range(num_trajectories):
                temp_trajectory, temp_ground_truth_state, temp_ground_truth_derivative = self.simulate_trajectory(
                    initial_condition=initial_conditions[trajectory_id],
                    times=times[dimension_id][trajectory_id],
                    sigma=sigmas[trajectory_id],
                    key=key[num_trajectories*dimension_id+trajectory_id])
                current_trajectory.append(temp_trajectory[:, dimension_id])
                current_ground_truth_state.append(temp_ground_truth_state[:, dimension_id])
                current_ground_truth_derivative.append(temp_ground_truth_derivative[:, dimension_id])
            trajectories.append(current_trajectory)
            ground_truth_states.append(current_ground_truth_state)
            ground_truth_derivatives.append(current_ground_truth_derivative)
