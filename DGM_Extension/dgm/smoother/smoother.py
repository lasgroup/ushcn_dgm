...
def get_smoother(
    ...
    def multi_d_smoother_apply(
            observation_times: List[jnp.array],  # List over dimensions of jnp.arrays of shape n_obs x 1
            matching_times: jnp.array,  # List over dimensinos of jnp.arrays of shape n_deriv_obs x 1
            ic_for_observation_times: List[jnp.array],  # List over dimensions of jnp.arrays of shape n_obs x n_dim
            ic_for_matching_times: jnp.array,  # List over dimensions of jnp.arrays of shape n_deriv_obs x n_dim
            observations: List[jnp.array],  # List over dimensions of jnp.arrays of shape n_dim x n_obs x 1
        ...
        def apply_smoother_in_dimension(dimension):
            # mean has to be recalculated, since its arguments are now dimension specific
            prior_means = mean_apply(
                observation_times[dimension],
                ic_for_observation_times[dimension],
                parameters["mean"], parameters["core"])

            prior_means_for_derivatives = mean_apply(
                matching_times[dimension],
                ic_for_matching_times[dimension],
                parameters["mean"], parameters["core"])

            prior_derivative_means = jnp.squeeze(vec_d_mean_apply(
                matching_times[dimension].reshape(-1),
                ic_for_matching_times[dimension],
                parameters["mean"], parameters["core"]))
        ...
        smoothed_means = []
        derivative_means = []
        derivative_covariances = []
        log_det_plus_obs = []

        for dim_id in range(n_dim):
            curr_results = apply_smoother_in_dimension(dim_id)
            smoothed_means.append(curr_results[0])
            derivative_means.append(curr_results[1])
            derivative_covariances.append(curr_results[2])
            log_det_plus_obs.append(curr_results[3])

        return smoothed_means, derivative_means, derivative_covariances, log_det_plus_obs

    @jax.jit
    def multi_d_smoother_posterior(
            evaluation_times: List[jnp.array],  # List over dimensions of jnp.arrays of shape n_test_obs x 1
            observation_times: List[jnp.array],  # List over dimensions of jnp.arrays of shape n_obs x 1
            ic_for_evaluation_times: jnp.array,  # List over trajectories of jnp.arrays  (n_test_obs x n_dim)
            ic_for_observation_times: jnp.array,  # List over trajectories of jnp.arrays  (n_obs x n_dim)
            observations: jnp.array,  # List over dimensions of jnp.arrays of shape n_dim x n_obs x 1
            parameters: pytree,  # type jax.pytree
    ):

        def apply_means_and_covariances_in_dimension(dimension):
            normal_prior_means = mean_apply(observation_times[dimension],
                                            ic_for_observation_times[dimension],
                                            parameters["mean"], parameters["core"])
            test_prior_means = mean_apply(evaluation_times[dimension],
                                          ic_for_evaluation_times[dimension],
                                          parameters["mean"], parameters["core"])
            derivative_prior_means = jnp.squeeze(
                vec_d_mean_apply(evaluation_times[dimension],
                                 ic_for_evaluation_times[dimension],
                                 parameters["mean"], parameters["core"]))
            derivative_prior_means = derivative_prior_means.reshape(-1, test_prior_means.shape[1])
            return get_means_and_covariances_test(
                evaluation_times[dimension],
                observation_times[dimension],
                ic_for_evaluation_times[dimension],
                ic_for_observation_times[dimension],
                observations[dimension],
        ...
        for dim_id in range(n_dim):
            curr_results = apply_means_and_covariances_in_dimension(dim_id)
            state_means.append(curr_results[0])
            state_variances.append(curr_results[1])
            derivative_means.append(curr_results[2])
            derivative_variances.append(curr_results[3])

        return state_means, state_variances, derivative_means, derivative_variances
...
def joint_smoother(kernel: KernelType, kernel_kwargs: Dict,
    ...
    def smoother_init(rng, num_dim):
        parameters = dict()
        parameters['noise_std'] = -20 * jnp.ones(num_dim)  # TODO: this was 2
